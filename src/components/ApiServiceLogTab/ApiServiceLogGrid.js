...
import ApiServiceLogDetail from './ApiServiceLogDetail';
import { Button } from 'primereact/button';
import moment from 'moment/moment';
import { jsonToFileDownload } from '../../util/ExcelUtil';

export default function ApiServiceLogGrid({env, ecommerce}) {
    ...
    const loadLazyData = async() => {
        setLoading(true);
        let resp = await tbkService.getListApiServiceLog(lazyState);
    ...
    const exportCsv = async() => {
        ...
        let resp = await tbkService.getListApiServiceLog(filter);
        if (resp.data){
            let labels = Object.keys(resp.data[0]);
            let names = Object.keys(resp.data[0]);
            jsonToFileDownload(labels, names, resp.data, `asl_${moment(new Date()).format('YYYY-MM-DD_HH:mm:ss')}.xlsx`);
        }
        setLoading(false);
    };
    ...
    return (
        ...
        {selectedRecord ? (
            <ApiServiceLogDetail env={env} ecommerce={ecommerce} recordId={selectedRecord.id} record={selectedRecord} />
        ) : (
            <></>
        )}
        </>
